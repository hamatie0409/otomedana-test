...
KIND_PROFILE = re.compile(
    # 見出しは連番つきのことが多い（【キャラ紹介①】【人物紹介・三】など）。
    # 連番の記号だけを許して、語そのものの列挙は変えない
    r"【\s*(?:Character|キャラクター紹介|攻略キャラクター紹介|キャラ紹介|登場人物情報|"
    r"新キャラクター紹介\S*|攻略キャラ\S*"
    # 【登場人物・一】【キャラクター情報】【新キャラクター情報】のように、
    # 「紹介」の語が無い見出しの紹介連載もある
    r"|登場人物|新?キャラクター情報|キャラクターデータ)"
    r"[\s・･:：0-9０-９①-⑳一二三四五六七八九十]{0,4}】"
    ...
    # 「◤キャラクターPickup║月城葦夜◢」のように、紹介連載の目印が
    # 「紹介」以外の語のこともある
    r"|キャラ(?:クター)?\s*(?:[Pp]ick\s?[Uu]p|ピックアップ)"
...
KIND_ICON = re.compile(
    r"アイコン[をのも]?\s*(?:配布|プレゼント|差し上げ)|推しアイコン"
    r"|壁紙[をのも]?\s*(?:配布|プレゼント)|ヘッダー[をのも]?\s*(?:配布|プレゼント)", re.I)
...
SUBJECT_HEAD = re.compile(
    ...
    r"|[^\n。]{0,12}紹介は\s*"
    # 「本日4月19日は不知火一樹♈の誕生日🎉」のような誕生日ポスト。
    # 本文に別のキャラ名が混ざっていても、主題は「◯月◯日は」の直後の人。
    r"|本日?\s*[0-9０-９]{1,2}\s*[月/／]\s*[0-9０-９]{1,2}\s*日?\s*は\s*"
    # ◤キャラクター紹介◢ のように、【】ではなく別の括りを使う書式もある。
    r"|[◤◢◆■●★☆＼／\\/]{1,3}[^\n]{0,16}(?:紹介|情報|Character|CHARACTER|Profile)"
    r"[^\n]{0,6}[◤◢◆■●★☆＼／\\/]{1,3}\s*)"
    # 【神凪ノ杜　登場人物紹介】●龍神奇譚攻略対象キャラ 東雲（CV.柿原徹也）のように、
    # 見出しのあとに「どの区分のキャラか」を●で書く書式がある。oEmbed は改行を消すので
    # 区分の見出しと名前がつながってしまう。区分の行も見出しの一部として読み飛ばす。
    r"(?:[●○][^●○【】「『\n]{0,16}\s*)?")


BIRTHDAY_HEAD = re.compile(r"本日?\s*[0-9０-９]{1,2}\s*[月/／]\s*[0-9０-９]{1,2}\s*日?\s*は\s*")
...
def subject_hit(text, name):
    ...
    for v in name_variants(name):
        ...
        for m in SUBJECT_HEAD.finditer(text):
            ...
            # 誕生日ポストは「本日11月17日は四皇學園生徒会会長・鉤貫レム様の誕生日！」
            # のように、日付と名前のあいだに肩書きが入る。中黒は先に潰しているので
            # 区切りが残らない。この書式だけは窓の中のどこにあっても主題とみなす。
            # 窓が短いので、別のキャラの名前を拾ってしまう心配は小さい。
            if BIRTHDAY_HEAD.fullmatch(m.group(0)):
                if fv in win[:SUBJECT_SPAN + len(fv)]:
                    return True
                continue
